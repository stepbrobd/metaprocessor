...
def get_preprocessed(task: pathlib.Path) -> pathlib.Path:
    ...
    if folder.glob("-Preprocessed.feather"):
        ...
        return folder/(folder.name+"-Preprocessed.feather")
    elif folder.glob("-Preprocessed.csv"):
        print(f"Using {folder/(folder.name+'-Preprocessed.csv')}")
        return folder/(folder.name+"-Preprocessed.csv")
    else:
        raise FileNotFoundError("No preprocessed data found")


def get_split_indices(task: pathlib.Path, config: dict) -> list:
    df = pd.read_feather(task)
    if len(df) < 10000:
        print(task, df.shape)
...
def uptime(task: pathlib.Path, config: dict) -> None:
    file = get_preprocessed(task)
    # data = pd.read_feather(file)
    get_split_indices(file, config)
